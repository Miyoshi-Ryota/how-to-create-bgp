...
イベントとしてはTCPコネクションの確立や、BGP OpenMessageの受信などを扱います。

以下に<<first_impl_bgp>>を図示します。図中の黒丸はステートマシンの初期状態を表しています。
...
Connect --> OpenSent : TCP Cr Acked | TCP Connection Confirmed event
note on link
   対向側機器にBGP OpenMessageを送信する。
end note
OpenSent --> OpenConfirm : BGP Open event
note on link
    対向機器にBGP Keepalive Messageを送信する。
end note
OpenConfirm --> Established : Keep Alive Msg event
